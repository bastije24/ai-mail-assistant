...
export const CreateAiTagDialog = ({ children, onTagCreate }: CreateAiTagDialogProps) => {
  ...
  const handleSave = () => {
    if (!tagName.trim() || !description.trim()) {
      toast({
        title: "Chyba",
        description: "Prosím vyplňte názov a popis tagu",
        variant: "destructive",
      });
      return;
    }
    ...
    if (keywords.length === 0) {
      toast({
        title: "Chyba", 
        description: "Pridajte aspoň jedno kľúčové slovo pre AI rozpoznávanie",
        variant: "destructive",
      });
      return;
    }
    ...
    toast({
      title: "Úspech",
      description: `AI tag "${tagName}" bol úspešne vytvorený`,
    });
